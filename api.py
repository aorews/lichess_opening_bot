...
class Lichess():
    def __init__(self, token):
        ...
        self.set_user_agent("opening_bot")
    ...
    @backoff.on_exception(backoff.constant,
        ...
        interval=0.1,
        giveup=is_final)
    def api_get(self, path, baseUrl = True):
        if baseUrl:
    ...
    def set_user_agent(self, username):
        self.header.update({"User-Agent": "lichess-bot/1 user:{}".format(username)})
        self.session.headers.update(self.header)
    ...
    def get_challenges(self):
        ongoing_games = self.api_get(ENDPOINTS["challenges"])
        return ongoing_games
    ...
    def get_lichess_database(self, moves):
        ...
        link = (
            ...
            "fen=rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR%20w%20KQkq%20-%200%201&"
            f"play={moves}"
        )
        moves = self.api_get(link, baseUrl=False)
        return moves